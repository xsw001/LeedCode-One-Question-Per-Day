        ...
        for (int i = 0; i < s.length(); i++) {
            if (s.charAt(i) != '?')
                ...
                if (i == 0) {
                    if (s.length() == 1)
                        return "a";
                    sb.append(arr[s.charAt(i + 1) == '?' ? 0 : (s.charAt(i + 1) - 'a' + 1) % 26]);
                } else {
                    if (i < s.length() - 1 && ((s.charAt(i + 1) - sb.charAt(i - 1) == 1) || (s.charAt(i + 1)=='a' && sb.charAt(i - 1) == 'z')))
                        sb.append(arr[(sb.charAt(i - 1) - 'a' + 2) % 26]);
                    else
                        sb.append(arr[(sb.charAt(i - 1) - 'a' + 1) % 26]);
                }
            }
        }
        return sb.toString();
