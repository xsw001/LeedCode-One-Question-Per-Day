...
public class medium_1482 {
    ...
    public static void main(String[] args) {
        int[] data = {5, 37, 55, 92, 22, 52, 31, 62, 99, 64, 92, 53, 34, 84, 93, 50, 28};
        ArrayList<String> list = new ArrayList<>();
        System.out.println(minDays(data, 8, 2));
    }
    ...
    public static int minDays1(int[] bloomDay, int m, int k) {
        ...
        for (int i = 0; i <= n - k - m + 1; i++) {
            ...
            for (int j = i; j <= n - k; j += k) {
                list.add(Arrays.stream(Arrays.copyOfRange(bloomDay, j, j + k)).max().getAsInt());
            }
            int size = list.size();
            if (size >= m) {
                for (int t = 0; t < m; t++) {
